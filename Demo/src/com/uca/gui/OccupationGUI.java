...
public class OccupationGUI {
    ...
    public static String deleteLocataire(Request request) throws IOException, TemplateException {
        ...
        OccupationCore.delete(new Occupation(numeroTel, "Locataire", numeroAppt, adresseAppt));
    ...
    public static String saveLocataire(Request request) throws IOException, TemplateException {
        ...
        OccupationCore.save(new Occupation(numeroTel, "Locataire", numeroAppt, adresseAppt));
        return getInfo(request);
    }

    public static String deleteProprietaire(Request request) throws IOException, TemplateException {
        Configuration configuration = _FreeMarkerInitializer.getContext();
        String numero = request.queryParams("numeroTel");
        numero = numero.replaceAll("[^\\d]", ""); // Supprime les caractères non numériques de la chaîne
        int numeroTel = Integer.parseInt(numero);
        ...
        OccupationCore.delete(new Occupation(numeroTel, "Proprietaire", numeroAppt, adresseAppt));
    ...
    public static String createProprietaire(Request request) throws IOException, TemplateException {
        Configuration configuration = _FreeMarkerInitializer.getContext();
        int numeroAppt = Integer.parseInt(request.queryParams("numeroAppt"));
        String adresseAppt = request.queryParams("adresseAppt");
        ...
        Writer output = new StringWriter();
        Template template = configuration.getTemplate("newproprietaire.ftl");
        template.setOutputEncoding("UTF-8");
        template.process(input, output);
        return output.toString();
    }

    public static String saveProprietaire(Request request) throws IOException, TemplateException {
        int numeroTel = Integer.parseInt(request.queryParams("numeroTel"));
        int numeroAppt = Integer.parseInt(request.queryParams("numeroAppt"));
        String adresseAppt = request.queryParams("adresseAppt");
        ...
        OccupationCore.save(new Occupation(numeroTel, "Propriétaire", numeroAppt, adresseAppt));
        return getInfo(request);
    }
}
